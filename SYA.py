...
class Interpreter:
    ...
    def traverse(self, node):
        if type(node) == list:
            self.variables.append({}) # inner scope enter
            
            rv = ''
            for statement in node:
                rv = self.traverse(statement)
            
            del self.variables[-1] # inner scope exit -> everything dies
            return rv
...
''') == 2
    ...
    assert interpreter.execute('((1+2)*3)') == 9
    assert interpreter.execute('017+1') == 18
